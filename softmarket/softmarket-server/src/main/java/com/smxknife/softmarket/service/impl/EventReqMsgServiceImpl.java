...
import com.smxknife.softmarket.common.comm.EventType;
...
@Service(WeChatConstant.EVENT_REQ_MSG_SERVICE)
public class EventReqMsgServiceImpl implements ReqMsgService {

	Logger logger = Logger.getLogger(EventReqMsgServiceImpl.class);
	...
	public String process(Map<String, String> requestMap) {
		...
		switch (EventType.valueOf(eventType)) {
			...
			case MASSSENDJOBFINISH:
				resContent = "MASSSENDJOBFINISH";
				resContent = textRespMsgService.sendMsg(requestMap, resContent);
				break;
			case subscribe:
				resContent = "subscribe";
				resContent = textRespMsgService.sendMsg(requestMap, resContent);
				break;
			case unsubscribe:
				resContent = "unsubscribe";
				resContent = textRespMsgService.sendMsg(requestMap, resContent);
				break;
				default:
					resContent = "请输入正确指令";
