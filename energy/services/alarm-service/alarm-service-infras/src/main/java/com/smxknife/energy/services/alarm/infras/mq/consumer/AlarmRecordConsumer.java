...
@Slf4j
@Component
public class AlarmRecordConsumer extends Thread {

	@Autowired
	private AlarmRecordConsumerProperties props;

	private KafkaConsumer<String, AlarmRecord> kafkaConsumer;
	...
	public void run() {
		while (true) {
			final ConsumerRecords<String, AlarmRecord> consumerRecords = kafkaConsumer.poll(Duration.ofMillis(0));

			consumerRecords.partitions().forEach(topicPartition -> {
				final List<ConsumerRecord<String, AlarmRecord>> records = consumerRecords.records(topicPartition);
				final List<AlarmRecord> recordList = records.stream().map(v -> v.value()).collect(Collectors.toList());
